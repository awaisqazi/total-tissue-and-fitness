...
- **Date:** 2026-09-23
- **Status:** Accepted (content details pending owner input)
- **Goal:** Follow the owner's direction (Josh, relayed 2026-09-23) that the top of the site put Manual Therapy together with Personal Training and Group Fitness Classes, alongside Contrast Therapy, so visitors looking to train find those offers straight away.
- **Decision:** `services` in `src/data/site.ts` now lists, in order, 01 Manual Therapy, 02 Personal Training, 03 Group Fitness Classes, 04 Contrast Therapy, typed as `ServiceEntry` with an optional `image`. Practitioner Mentorship moves out of `services` into an exported `practitionerProgram` (same shape) and renders as its own "For practitioners" split band on the home page, after the recovery section and before the couples workshop. The two training entries use short call-to-ask copy only and link to a new `/training/` route, which has a PageHero without an image, two offer cards with `tel:` links, a "How it fits" section that restates only existing facts (tissue work, movement education and contrast therapy are already offered; training sits alongside them), and the shared CTA. Cards without an `image` render a `.service-card-band` gradient panel with the outlined index number instead of a photo. The home services grid is 2 × 2 from 761 px up (the phone snap row is unchanged). The home hero eyebrow, description and middle fact ("Train" / "PERSONAL & GROUP") now name training. Desktop and mobile navigation and the footer add Training after Manual Therapy, and a FAQ entry answers whether training and classes are offered, pointing to `site.phoneDisplay`. The Contrast Therapy page eyebrow becomes "04 / CONTRAST THERAPY" and Mentorship "FOR PRACTITIONERS / MENTORSHIP" to match the new numbering.
- **Rationale:** The owner asked for the reprioritization. The only sourced facts are that the public Vagaro listing's business type is "Personal Trainer" and that it has a class cancellation policy; Vagaro currently lists no training or class services and the archived site has none. Presenting the offers as "call to ask" keeps them visible without inventing descriptions, pricing, schedules, formats, durations or outcomes, and the phone is the only path that works today. A non-photo band avoids implying the facility or contrast photos show training. 4-up cards were rejected because titles such as "Group Fitness Classes" wrap awkwardly in ~280 px cards at 1200 px; 2 × 2 keeps the photo bands legible.
- **Tradeoffs:** Training has no online booking path until Vagaro has matching services or classes, so `/book/` and the CTA still lead to manual therapy and contrast booking. The contact form's interest select was not extended: `workers/contact/index.js` validates `interest` against a fixed `allowedInterests` set, and the Google Form question behind it has fixed options, so new values would be rejected (400) until both are updated. Training inquiries use the phone, or "A general question" on the form.
- **Affected:** `/`, `/training/` (new, in the sitemap), `/contrast-therapy/`, `/mentorship-program/`, all routes' header and footer; `src/data/site.ts` (`ServiceEntry`, `services`, `practitionerProgram`, `faqs`), `src/pages/{index,training,contrast-therapy,mentorship-program,sitemap.xml}`, `src/components/{Navigation,Footer}.astro`, `src/styles/global.css`, `src/scripts/reveal.ts`, `tests/site.test.mjs` (sitemap count 8), README, content map.
- **Validation:** `npm run format`; root and Pages (`SITE_URL=https://awaisqazi.github.io BASE_PATH=/total-tissue-and-fitness PUBLIC_SITE_INDEXABLE=false`) `npm run validate`. Browser-pane checks at 390 and 1280 px of the home hero and services grid, `/training/`, desktop and mobile navigation, and the contact select, with no page-level horizontal overflow.
- **Unresolved (owner: Josh):** approved descriptions for Personal Training and Group Fitness Classes; pricing; class schedule, format and session length; Vagaro service or class listings so the booking widget can book them (then add a booking CTA to `/training/`); training photos to replace the non-photo bands; and, to offer "Personal Training" and "Group Fitness Classes" in the contact form, add those options to the Google Form's interest question first, then to `allowedInterests` in the Worker (redeploy) and to the `/contact/` select and `interestMap` (`training`, `classes`).
